...
//  Save The Red Dots
//
//  Created by Jeffrey Ford on 6/16/20.
...
//Top Level will be 100
class StoryMode: SKScene, SKPhysicsContactDelegate{
    ...
    let levelCompleteLabel = SKLabelNode(fontNamed: "Futura")
    let returnToMainMenuLabel = SKLabelNode(fontNamed: "Futura")
    let speedIncreaseLabel = SKLabelNode(fontNamed: "Futura")
    ...
    override func didMove(to view: SKView) {
        ...
        //Make sure the below code is commented out when ready
        //defaults.set(0, forKey: "levelSaved")
        ...
        levelLabel.position = CGPoint(x: self.size.width*0.5, y: self.size.height*0.75)
    ...
    func runLevelComplete(){
        ...
        var duration = 4.0
        var levelCompleteText = "Level Complete!"
        if level < 100 {
            level += 1
            
        } else{
            levelCompleteText = "Game Complete!\n\nNow go get a high\nscore in arcade\nmode!!!!"
            duration = 10.0
        }
        ...
        let changeSceneAction = SKAction.run(restartScene)
        let waitToChangeScene = SKAction.wait(forDuration: duration)
        let changeSceneSequence = SKAction.sequence([waitToChangeScene, changeSceneAction])
        self.run(changeSceneSequence)
        
        levelCompleteLabel.horizontalAlignmentMode = SKLabelHorizontalAlignmentMode.center

        levelCompleteLabel.numberOfLines = 0
        levelCompleteLabel.text = levelCompleteText
        levelCompleteLabel.fontSize = 80
        levelCompleteLabel.fontColor = SKColor.white
        levelCompleteLabel.position = CGPoint(x: self.size.width*0.5, y: self.size.height*0.5)
        levelCompleteLabel.zPosition = 1
        levelCompleteLabel.alpha = 0
        self.addChild(levelCompleteLabel)
        let fadeInAction = SKAction.fadeIn(withDuration: 0.6)
        levelCompleteLabel.run(fadeInAction)
        
        let defaults = UserDefaults()
        defaults.set(level, forKey: "levelSaved")
        
        
        
    }

    func printSpeedIncrease(){
        speedIncreaseLabel.text = "Speed of dot has increased!"
        speedIncreaseLabel.fontSize = 60
        speedIncreaseLabel.fontColor = SKColor.blue
        speedIncreaseLabel.position = CGPoint(x: self.size.width*0.5, y: self.size.height*0.7)
        speedIncreaseLabel.zPosition = 1
        speedIncreaseLabel.alpha = 0
        self.addChild(speedIncreaseLabel)
        let fadeInAction = SKAction.fadeIn(withDuration: 0.6)
        speedIncreaseLabel.run(fadeInAction)
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if gameStarted == false{
            ...
            traverseDuration = ((1152.0 - Double(level*7))*0.005)
            ...
            let SpawnDelayForSpawnCount = SKAction.repeat(SpawnDelay, count: spawnCount)
            self.run(SpawnDelayForSpawnCount)
            
            //print("hi")
            
            let distanceX = CGFloat(self.frame.width + wallPair.frame.width - marginSize * 2)
           
            let movePipesX = SKAction.moveBy(x: -distanceX, y: 0.0, duration: TimeInterval(traverseDuration))
            ...
            let movePipesY = SKAction.moveBy(x: 0.0, y: -distanceY, duration: TimeInterval(traverseDuration))
